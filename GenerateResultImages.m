...
for DatasetIndex = 1 : datasetsNum
    ...
    resultsFolder = ['./Results/LF-LGFA/', DatasetName, '/'];
    ...
    for iScene = 1 : sceneNum
        ...
        for u = 1 : angRes
            for v = 1 : angRes                
                ...
                % Save HR
                SavePath = ['./SRimages/HR/', DatasetName, '/', sceneName, '/'];
                if exist(SavePath, 'dir')==0
                    mkdir(SavePath);
                end
                imwrite(uint8(255*imgHR_rgb), [SavePath, num2str(u,'%02d'), '_', num2str(v,'%02d'), '.png' ]);

                 % Save SR
                SavePath = ['./SRimages/LF-LGFA/', DatasetName, '/', sceneName, '/'];
                if exist(SavePath, 'dir')==0
                    mkdir(SavePath);
                end
