...
def extract_image_patches(images, ksizes, strides, rates, padding='same'):
    ...
    if padding == 'same':
        images, paddings = same_padding(images, ksizes, strides, rates)  # torch.Size([25, 16, 131, 131]) (1, 2, 1, 2)
    elif padding == 'valid':
        pass
    else:
    ...
    unfold = torch.nn.Unfold(kernel_size=ksizes,
                             padding=0,
                             stride=strides)
    patches = unfold(images)  # torch.Size([25, 784, 1024])
...
class Global_Aggregation(nn.Module):
    ...
    def forward(self, b):  # B N C H W   2, 5, 96, 128, 128
        # get shapes
        output = []
        for B in range(b.shape[0]):
            # print(B)
            b_one = b[B]  # torch.Size([5, 96, 128, 128])

            kernel = self.ksize  # 7
            b1 = self.g(b_one)  # torch.Size([5, 16, 128, 128])
            b2 = self.theta(b_one)  # torch.Size([5, 16, 128, 128])
            b3 = self.phi(b_one)  # torch.Size([5, 16, 128, 128])

            raw_int_bs = list(b1.size())  # b*c*h*w #  [5, 16, 128, 128]

            patch_28, paddings_28 = extract_image_patches(b1, ksizes=[self.ksize, self.ksize],
                                                          strides=[self.stride_1, self.stride_1],
                                                          rates=[1, 1],
                                                          padding='same')  # torch.Size([5, 784, 1024]) (1, 2, 1, 2)
            patch_28 = patch_28.view(raw_int_bs[0], raw_int_bs[1], kernel, kernel,
                                     -1)  # torch.Size([5, 16, 7, 7, 1024])
            patch_28 = patch_28.permute(0, 4, 1, 2, 3)  # torch.Size([5, 1024, 16, 7, 7])


            patch_112, paddings_112 = extract_image_patches(b2, ksizes=[self.ksize, self.ksize],
                                                            strides=[self.stride_2, self.stride_2],
                                                            rates=[1, 1],
                                                            padding='same')  # torch.Size([25, 784, 16384]) (3, 3, 3, 3)
            patch_112 = patch_112.view(raw_int_bs[0], raw_int_bs[1], kernel, kernel,
                                       -1)  # torch.Size([25, 16, 7, 7, 16384])
            patch_112 = patch_112.permute(0, 4, 1, 2, 3)  # torch.Size([5, 1024, 16, 7, 7])
            ...
            patch_112_2 = patch_112_2.view(raw_int_bs[0], raw_int_bs[1], kernel, kernel,
                                           -1)  # torch.Size([25, 16, 7, 7, 16384])
            patch_112_2 = patch_112_2.permute(0, 4, 1, 2, 3)  # torch.Size([5, 1024, 16, 7, 7])


            _, paddings = same_padding(torch.split(b3, 1, dim=0)[0], [self.ksize, self.ksize], [1, 1],
                                       [1, 1])  # (3, 3, 3, 3)

            q = patch_28.contiguous().view(patch_28.shape[0] * patch_28.shape[1], -1)  # torch.Size([5*1024, 784])
            k = patch_112_2.permute(2, 3, 4, 0, 1)  # torch.Size([ 16, 7, 7,5,1024])
            k = k.contiguous().view(-1, k.shape[3] * k.shape[4])  # torch.Size([784, 5*1024])
            score_map = torch.matmul(q, k)  # torch.Size([5*1024, 5*1024])  # q*k

            b_s, l_s, h_s, w_s = b_one.shape[0], patch_28.shape[1], b_one.shape[2], b_one.shape[3]  # 5 1024 128 128
            att = F.softmax(score_map * self.softmax_scale, dim=1)  # torch.Size([5*1024, 5*1024])
            v = patch_112.contiguous().view(patch_112.shape[0] * patch_112.shape[1], -1)  # torch.Size([5*1024, 784])
            attMulV = torch.mm(att, v)  # torch.Size([5*1024, 784])  # att*V

            zi = attMulV.view(b_s, l_s, -1).permute(0, 2, 1)  # torch.Size([25, 784, 1024])
            zi = torch.nn.functional.fold(zi, (raw_int_bs[2], raw_int_bs[3]), (kernel, kernel), padding=paddings[0],
                                          stride=self.stride_1)  # torch.Size([25, 16, 128, 128])

            inp = torch.ones_like(zi)  # torch.Size([25, 16, 128, 128])
            inp_unf = torch.nn.functional.unfold(inp, (kernel, kernel), padding=paddings[0],
                                                 stride=self.stride_1)  # torch.Size([25, 784, 1024])
            out_mask = torch.nn.functional.fold(inp_unf, (raw_int_bs[2], raw_int_bs[3]), (kernel, kernel),
                                                padding=paddings[0],
                                                stride=self.stride_1)  # torch.Size([25, 16, 128, 128])

            zi = zi / out_mask  # torch.Size([25, 16, 128, 128])
            y = self.W(zi)  # torch.Size([25, 96, 128, 128])
            y = b_one + y
            if self.add_SE:
                y_SE = self.SE(y)
                y = self.conv33(torch.cat((y_SE * y, y), dim=1))
            output.append(y)  # 25, 16, 128, 128
        output = torch.stack(output, dim=0)  # torch.Size([8,25, 16, 128, 128])
        return output
...
if __name__ == "__main__":
    Global_Aggregation = Global_Aggregation(in_channels=36).cuda()
    feature = torch.randn(4, 5, 36, 128, 128).cuda()  # b,n c h w
    out = Global_Aggregation(feature)  # torch.Size([8,5, 96, 128,128])
    print(out.shape)
